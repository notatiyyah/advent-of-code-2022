...
def calculate_move_points(moves):
...
def generate_move(line):
    moves = line.strip("\n").split(" ")
    if moves[1] == "Y": moves[1] = moves[0]
    elif moves[1] == "Z": moves[1] = win_conditions[moves[0]]
    else: moves[1] = dict([reversed(i) for i in win_conditions.items()])[moves[0]]

    return moves

def calculate_round_points(line, ver):
    moves = split_line(line) if ver == 1 else generate_move(line)
    return calculate_move_points(moves) + move_points[moves[1]]
